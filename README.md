...
- **遊戲管理系統 (v0.4.0 重構)**
  - **`GameManagerService`**: 作為遊戲總管，負責協調遊戲核心服務，並初始化、管理整體的遊戲狀態。
  - **`GameLoopService`**: 驅動遊戲的主要循環，負責處理需要時間推進的事件，例如作物生長、動物產出等。
  - **`SaveLoadService`**: 專職處理遊戲的存檔與讀檔，使用 LocalStorage 保存遊戲進度。
  - **`GameDataService`**: 集中管理玩家的遊戲數據，例如金錢、時間等。
